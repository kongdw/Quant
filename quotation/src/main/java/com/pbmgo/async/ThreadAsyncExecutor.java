...
public class ThreadAsyncExecutor implements AsyncExecutor {
    ...
    @Override
    public <T> AsyncResult<T> startProcess(Callable<T> task, AsyncCallback<T> asyncCallback) {
        CompletableAsyncResult<T> result = new CompletableAsyncResult<>(asyncCallback);
        new Thread(() -> {
            try {
    ...
    @Override
    public <T> T endProcess(AsyncResult<T> asyncAsyncResult) throws ExecutionException, InterruptedException {
        if (asyncAsyncResult.isCompleted()) {
            return asyncAsyncResult.getValue();
        } else {
            asyncAsyncResult.await();
            return asyncAsyncResult.getValue();
        }
    }
     ...
     */
    private static class CompletableAsyncResult<T> implements AsyncResult<T> {
        ...
        CompletableAsyncResult(AsyncCallback<T> asyncCallback) {
